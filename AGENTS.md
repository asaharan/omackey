...
- `install.sh` symlinks this checkout into `~/.config/omarchy/plugins/asaharan.omackey`, optionally copies the Hyprland module to `~/.config/hypr/omackey.lua` (if switcher is enabled), enables the plugin, and reloads Hyprland. Preserve unrelated user configuration.
...
- If installation reaches plugin enablement before the shell discovers the plugin, restart the shell, run `omarchy-shell shell rescanPlugins`, then enable `asaharan.omackey` and verify it appears in `omarchy plugin list --json`.
